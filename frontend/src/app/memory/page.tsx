        ...
        <div className="space-y-8">
          ...
          <div className="glass-card p-4 flex flex-col md:flex-row items-center justify-between gap-4 border-dashed border border-blue-500/20 bg-blue-950/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-semibold text-[var(--text-primary)]">Active Synchronizer</h4>
            ...
            <div className="flex items-center gap-4 text-xs font-mono text-[var(--text-muted)]">
              <div className="flex items-center gap-2 bg-[var(--inner-box-bg)] px-3 py-1.5 rounded border border-[var(--inner-box-border)]">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div className="xl:col-span-2 space-y-6">
              <div className="glass-card p-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--inner-box-border)] pb-4 mb-4">
                  <div className="flex items-center gap-2">
                    <FileText className="w-5 h-5 text-cyan-400" />
                    <h3 className="font-semibold text-[var(--text-primary)]">Local Memory Workspace (.nova)</h3>
                  </div>
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded text-[10px] font-mono border transition-all ${
                          selectedFilename === file.filename
                            ? 'bg-blue-600/10 border-blue-500/40 text-blue-500 font-bold'
                            : 'bg-[var(--inner-box-bg)] border-[var(--inner-box-border)] text-[var(--text-muted)] hover:text-[var(--text-primary)]'
                        }`}
                      >
                        {file.filename}
                      </button>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-xs text-[var(--text-muted)]">
                    <span className="font-mono text-cyan-500 font-semibold">{selectedFilename}</span>
                    <div className="flex gap-2">
                      <button
                        onClick={() => setIsEditingFile(!isEditingFile)}
                        className="px-3 py-1 bg-[var(--inner-box-bg)] hover:bg-purple-50 dark:hover:bg-slate-800 border border-[var(--inner-box-border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded"
                      >
                  ...
                  <textarea
                    ...
                    className="w-full h-80 bg-[var(--inner-box-bg)] text-[var(--text-primary)] font-mono text-xs p-4 rounded-lg border border-[var(--inner-box-border)] outline-none focus:border-cyan-500/80 resize-none disabled:opacity-85"
                  />
                </div>
              </div>
              ...
              <div className="glass-card p-6">
                <div className="flex items-center gap-2 border-b border-[var(--inner-box-border)] pb-3 mb-4">
                  <FileCheck className="w-5 h-5 text-emerald-400" />
                  <h3 className="font-semibold text-[var(--text-primary)]">Cloud Decisions Database</h3>
                </div>
                ...
                {decisions.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {decisions.map((dec, idx) => (
                      <div key={idx} className="p-4 bg-[var(--inner-box-bg)] rounded-lg border border-[var(--inner-box-border)] text-xs">
                        <div className="flex justify-between items-start gap-4">
                          <h4 className="font-bold text-[var(--text-primary)] text-sm">{dec.title}</h4>
                          <span className="text-[10px] font-mono text-[var(--text-muted)]">
                            {new Date(dec.timestamp).toLocaleDateString()}
                          </span>
                        </div>
                        <p className="text-[var(--text-secondary)] mt-2 leading-relaxed">{dec.content}</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 pt-3 border-t border-[var(--inner-box-border)] text-[10px] text-[var(--text-muted)] leading-relaxed">
                          <div>
                            <span className="font-semibold text-[var(--text-muted)] block uppercase font-mono mb-0.5">RATIONALE</span>
                            <span>{dec.rationale}</span>
                          </div>
                          <div>
                            <span className="font-semibold text-[var(--text-muted)] block uppercase font-mono mb-0.5">IMPACT</span>
            ...
            <div className="glass-card p-6 h-fit">
              <div className="flex items-center gap-2.5 border-b border-[var(--inner-box-border)] pb-3 mb-4">
                ...
                <h3 className="font-semibold text-[var(--text-primary)]">Log Decision</h3>
              </div>

              <form onSubmit={handleCreateDecision} className="space-y-4">
                <div>
                  <label className="text-[10px] text-[var(--text-muted)] block mb-1 font-mono">CHOICE TITLE</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-[10px] text-[var(--text-muted)] block mb-1 font-mono">DECISION DETAILS</label>
                  <textarea
                    required
                    rows={3}
                    ...
                    className="w-full bg-[var(--inner-box-bg)] border border-[var(--inner-box-border)] text-[var(--text-primary)] text-xs rounded-md px-3 py-2 outline-none focus:border-cyan-500/80 placeholder-[var(--text-muted)] resize-none"
                  />
                </div>

                <div>
                  <label className="text-[10px] text-[var(--text-muted)] block mb-1 font-mono">TECHNICAL RATIONALE</label>
                  <textarea
                    rows={2}
                ...
                <div>
                  <label className="text-[10px] text-[var(--text-muted)] block mb-1 font-mono">ARCHITECTURAL IMPACT</label>
                  <input
                    type="text"
